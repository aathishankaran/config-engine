...
function derivePreviousDayInfo(node) {
  ...
  if (!inputNode) {
    ...
    if (srcNode) {
      /* Find input connected to this step */
      var visited = {};
        ...
        '<div id="pv-prev-day-section"' + (hasPrevDay ? '' : ' style="display:none;"') + '>' +
          '<div class="pv-feature-card-body">' +
            '<p style="font-size:11px;color:#64748b;margin:0 0 6px">' +
              'Previous business day is <strong>auto-calculated at runtime</strong> (skips holidays).' +
            '</p>' +
            prevDayBanner +
          '</div>' +
        '</div>' +
  ...
  if ($btnFromSource.length) {
    $btnFromSource.on('click', function() {
      ...
      node.ctrl_file_fields = srcFields.map(function(f) {
        return { name: f.name || '', type: (f.type || 'STRING').toUpperCase(), expression: '' };
      });
  ...
  $input.on('change', function() {
    ...
    var applyCtrlFields = function(parsedFields, filename) {
      ...
      node.ctrl_file_fields = parsedFields.map(function(f) {
        ...
        return { name: f.name || '', type: (f.type || 'STRING').toUpperCase(), expression: existing ? (existing.expression || '') : '', length: f.length || 0 };
      });
    ...
    $btn.on('click', function() {
      ...
      if (action === 'add-field') {
        ...
        if (ns === 'po-header-fields') {
          ...
          node.ctrl_file_fields.push({name:'', type:'STRING', expression:''});
        }
