  ...
  steps.forEach(function(s) {
    ...
    if (s.type === 'filter') {
      ...
      n.ctrl_file_fields  = (logic.ctrl_file_fields || []).map(function(f) {
        return { name: f.name || '', type: (f.type || 'STRING').toUpperCase(), expression: f.expression || '', length: f.length || 0 };
      });
    ...
    if (fileInput.files && fileInput.files[0]) {
      ...
      reader.onload = function(e) {
        try {
          ...
          DS.fn.toast('Loaded from file', 'success');
        } catch(err) { DS.fn.toast('Invalid JSON: ' + err.message, 'error'); }
      };
      ...
      try {
        ...
        $('#import-modal').addClass('hidden');
        DS.fn.toast('Loaded from JSON', 'success');
      } catch(err) { DS.fn.toast('Invalid JSON: ' + err.message, 'error'); }
    }
  });
