...
def api_save_node_test_file(filename):
    ...
    try:
        ...
        if not file_format or (file_format == "FIXED" and not fields_json):
            try:
        ...
        try:
            ...
            # Control-file expected uploads: parse as CSV with ctrl_file_fields as headers.
            # The user uploads a CSV whose first row is ctrl field names
            # (COUNT-RECORDS, EFFECTIVE-RECORD, …) so reconciliation columns match
            # the generated ctrl output parsed with the same field schema.
            if node_name.startswith("__ctrl__"):
                ...
                # ── Normalize absolute start positions ─────────────────────────
                # In multi-record copybooks the parser assigns DATA fields absolute
                # positions (e.g. start=121) relative to the combined record layout,
                # but each physical DATA record starts at byte 1.  Detect this by
                # checking whether the minimum start position exceeds the line length
                # and subtract the offset so positions become record-relative.
                start_adj = 0
