...
def _write_fixed_input(rows: list[dict], path: Path, cfg: dict) -> None:
    """
    ...
    for _ in range(trailer_count):
        lines.append(_make_line({}, trailer_fields) if trailer_fields else " " * line_width)

    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + ("\n" if lines else ""), encoding="utf-8")


def _write_input_file(name: str, rows: list[dict], inp_cfg: dict, input_dir: Path) -> str:
    """
    ...
    if fmt == "FIXED":
        ...
        _write_fixed_input(rows, input_dir / file_name, inp_cfg)
